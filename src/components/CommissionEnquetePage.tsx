...
// Interface pour une commission d'enquête
interface CommissionEnqueteItem {
  id: string;
  numero: string;
  objet: string;
...
// Données fictives pour les commissions d'enquête basées sur l'image fournie
const commissionsEnquete: CommissionEnqueteItem[] = [
  {
    id: 'ce01',
    numero: '01',
    objet: 'Gestion de la Covid 19 par le Gouvernement',
    deputesMembres: [
      'Madame Angélique NGOMA',
      'Monsieur Ruffin Pacôme ONDZOUNGA',
      'Monsieur Séraphin AKURE DAVAIN',
      'Monsieur Minault Maxime ZIMA EBEYARD',
      'Monsieur Aurélien NTOUTOUME MEBIAME',
      'Monsieur Jean Claude MBOUMI NZINZI',
      'Monsieur Pacôme Grégoire MOUBELET BOUBEYA',
      'Monsieur Jean Bosco MBAGOU',
      'Monsieur Jean François NDONGOU',
      'Monsieur Jean Pierre DOUKAGA KASSA',
    ],
    administration: [
      'Messieurs Gabin MOUSSAVOU, Directeur de la Séance',
      'Et Roy LEWANGA, Administrateur',
    ],
    legislature: 'TREIZIEME LEGISLATURE',
    session: 'Seconde session ordinaire',
    annee: '2020',
  },
  // Ajoutez d'autres commissions si nécessaire
];
...
const CommissionEnquetePage: React.FC<CommissionEnquetePageProps> = ({ navigateTo }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filteredCommissions, setFilteredCommissions] = useState(commissionsEnquete);
  const [currentArticleIndex, setCurrentArticleIndex] = useState(0); // Pour la sidebar "À la une"

  useEffect(() => {
    const current = commissionsEnquete.filter(commission =>
        ...
        <div className="w-full lg:w-9/12 px-4">
          ...
          <nav className="text-sm text-gray-500 mb-8" aria-label="Breadcrumb">
            <ol className="list-none p-0 inline-flex">
              ...
              <li className="flex items-center">
                <span className="text-gray-900">Commissions d'enquête</span>
              </li>
            </ol>
          </nav>

          <h1 className="text-3xl font-bold text-gray-800 mb-8 font-serif">Commissions d'enquête</h1>
          ...
          <div className="mb-8 flex items-center border border-gray-300 rounded-lg shadow-sm overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
            <input
              type="text"
              placeholder="Rechercher une commission par objet, membre ou année..."
          ...
          {filteredCommissions.length > 0 ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-6 text-center text-gray-600">
              Aucune commission d'enquête trouvée pour les critères de recherche.
            </div>
          )}
        </div>
...
export default CommissionEnquetePage;
